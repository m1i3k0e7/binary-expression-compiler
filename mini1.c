...
AST *parse(Token *arr, int l, int r, GrammarState S) {
	...
	switch (S) {
		...
		case UNARY_EXPR:
			if (arr[l].kind == PREDEC || arr[l].kind == PREINC || arr[l].kind == PLUS || arr[l].kind == MINUS)
            {
...
void codegen(AST *root)
{
    ...
    else if(root->kind == PREINC || root->kind == PREDEC) {
        AST *tmp = root->mid;
        while(tmp->kind == LPAR) {
            tmp = tmp->mid;
        }
    ...
    else if(root->kind == POSTINC || root->kind == POSTDEC) {
        AST *tmp = root->mid;
        while(tmp->kind == LPAR) {
            tmp = tmp->mid;
        }
